...
@section('content')
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 card-hover animate-fade-in-up">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-red-400 rounded-lg flex items-center justify-center animate-float">
                    <i class="fas fa-users text-white text-xl"></i>
                </div>
                <!-- Optional: Percentage change if available -->
            </div>
            <h3 class="theme-text-secondary text-sm mb-1">Total Pengguna</h3>
            <p class="text-3xl font-bold text-red-400">{{ number_format($stats['total_users']) }}</p>
        ...
        <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 card-hover animate-fade-in-up delay-100">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-red-500 rounded-lg flex items-center justify-center animate-float">
                    <i class="fas fa-box text-white text-xl"></i>
                </div>
            </div>
            <h3 class="theme-text-secondary text-sm mb-1">Total Produk</h3>
            <p class="text-3xl font-bold text-red-400">{{ number_format($stats['total_products']) }}</p>
            <div class="mt-4 w-full theme-bg-secondary rounded-full h-2">
        ...
        <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 card-hover animate-fade-in-up delay-200">
            <div class="flex items-center justify-between mb-4">
                <div class="w-12 h-12 bg-red-600 rounded-lg flex items-center justify-center animate-float">
                    <i class="fas fa-newspaper text-white text-xl"></i>
                </div>
            </div>
            <h3 class="theme-text-secondary text-sm mb-1">Total Artikel</h3>
            <p class="text-3xl font-bold text-red-400">{{ number_format($stats['total_articles']) }}</p>
             <div class="mt-4 w-full theme-bg-secondary rounded-full h-2">
        ...
        <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 card-hover animate-fade-in-up delay-300">
            ...
            <p class="text-3xl font-bold text-red-400">{{ number_format($stats['total_gallery_images']) }}</p>
            <div class="mt-4 w-full theme-bg-secondary rounded-full h-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 animate-fade-in-up delay-400">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-lg font-bold text-red-400">Artikel Terbaru</h2>
                <a href="{{ route('admin.artikels.index') }}" class="text-sm theme-text-secondary hover:text-red-400 transition-colors">Lihat Semua</a>
            </div>
            <div class="space-y-4">
                @forelse($recent_articles as $article)
                    <div class="flex items-center justify-between p-3 rounded-lg hover:bg-red-400/10 transition-all duration-300 theme-border border-b last:border-0 border-dashed">
        ...
        <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 animate-fade-in-up delay-500">
             <div class="flex justify-between items-center mb-6">
                <h2 class="text-lg font-bold text-red-400">Produk Terbaru</h2>
                <a href="{{ route('admin.produk.index') }}" class="text-sm theme-text-secondary hover:text-red-400 transition-colors">Lihat Semua</a>
            </div>
            <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <!-- Popular Articles -->
         <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 animate-fade-in-up delay-600">
             <div class="flex justify-between items-center mb-6">
                <h2 class="text-lg font-bold text-red-400">Artikel Populer</h2>
            </div>
            <div class="space-y-4">
    ...
    <div class="bg-gradient-card backdrop-blur-sm theme-border border rounded-xl p-6 animate-fade-in-up delay-700">
         <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-bold text-red-400">Aktivitas Terbaru</h2>
            <a href="{{ route('admin.activity-logs.index') }}" class="text-sm theme-text-secondary hover:text-red-400 transition-colors">Lihat Semua</a>
        </div>
        <div class="space-y-4">
             @forelse($recent_activities as $activity)
                <div class="flex items-start p-3 rounded-lg hover:bg-red-400/10 transition-all duration-300 theme-border border-b last:border-0 border-dashed">
                     <div class="flex-shrink-0 w-8 h-8 rounded-full bg-red-400/20 flex items-center justify-center mr-3">
                        <i class="fas fa-user text-red-400 text-sm"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                         <p class="text-sm theme-text-primary">
                            <span class="font-medium text-red-400">{{ $activity->user->nama_lengkap ?? 'System' }}</span>
                            <span class="theme-text-secondary">{{ $activity->action }}</span>
                            <span class="font-medium text-red-400">{{ $activity->table_name }}</span>
                        </p>
